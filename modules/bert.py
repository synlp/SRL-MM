...
#     """
#     def __init__(self, config, num_labels):
#         super(BertForTokenClassificationCRF, self).__init__(config)
#         self.num_labels = num_labels
#         self.bert = BertModel(config)
#         self.dropout = nn.Dropout(config.hidden_dropout_prob)
#         self.classifier = nn.Linear(config.hidden_size, num_labels)
...
#
#
# class BertForTokenClassificationBiLSTMCRF(BertPreTrainedModel):
#     """BERT model for token-level classification.
#     This module is composed of the BERT model with a linear layer on top of
#     the full hidden state of the last layer.
#
...
#     ```
#     """
#     def __init__(self, config, num_labels, lstm_hidden_size=200, lstm_num_layers=1):
#         super(BertForTokenClassificationBiLSTMCRF, self).__init__(config)
#         self.num_labels = num_labels
#         self.bert = BertModel(config)
#         self.dropout = nn.Dropout(config.hidden_dropout_prob)
#         self.lstm = BiLSTM(input_size=config.hidden_size, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers,
#                            batch_first=True)
...
#             return -1 * loss
#         else:
#             tags = self.crf.decode(logits, attention_mask).squeeze(0)
#             return tags
